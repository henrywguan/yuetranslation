...
### Other app rate limits (2026-09-19)

| Bucket | Endpoint | Default | Key |
| --- | --- | --- | --- |
| pushSubscribe | `POST /api/push/subscribe` (+ unsubscribe) | `YUE_PUSH_SUBSCRIBE_RL_PER_MIN=20` | Client IP |
| harborPut | `PUT /api/harbor-quest` | `YUE_HARBOR_PUT_RL_PER_MIN=120` | Signed-in user |
| harborGift | `POST /api/harbor-quest/gift` | `YUE_HARBOR_GIFT_RL_PER_MIN=30` | Signed-in user |
...
| `practice_partner_count` / `practicePartnerCount` | Each Practice Partner LLM drill turn (`POST /api/admin/practice-partner/chat`) from Admin or Account Hub `#/practice` | None — admin Users table + usage detail only |
