...
2. **Upload image** — still photo; draw boxes and/or auto-detect; **Translate all** optional. Jade/glass overlays stay locked to each placed/OCR word region (not dragged after placement; not collision-nudged so dense menus stay on-word). Mouse-wheel zoom (and slider); overlay text scales in screen space so it stays crisp. Selectable Results list for every region.
