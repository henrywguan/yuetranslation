...
| Practice Partner | Live lab: Harbor orb + captions. Duolingo-style say-this — pick a deck (**Animals / Foods / Common phrases / Expert phrases**), 港灣 demands a line in that category (English + 漢字 + Jyutping), you speak, the model judges, then advances or retries. A pass plays a short chime + jade check (no congratulatory text) and appends to a **High scores** log (`localStorage` `yue-practice-partner-scores-v1`) revealed from the Practice Partner page / admin tab. Mic → Web Speech STT → DeepSeek → existing Azure TTS. No Voice Live / Foundry. Signed-in Account Hub carousel (Harbor Quest ↔ Practice Partner) opens `#/practice`. Chat still uses the admin route and meters `practice_partner_count` (view-only). |
