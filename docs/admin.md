...
- `POST /api/admin/practice-partner/chat` — admin Practice Partner say-this drill (DeepSeek); body `{ messages, activeDrill? }`; returns `{ reply, drill }` (TTS line + verdict / next phrase); meters `practice_partner_count` (view-only)
- `GET /api/admin/me`
