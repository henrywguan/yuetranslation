...
| Character GLB | Canoe + land Scout plant authored Meshy v7 GLBs. They are **unskinned A-pose** — `harborScoutRig.ts` paints the armature. Do not assume T-pose. Frozen fishing / sit / arm swing: [`GLB-ANIMATION.md`](./GLB-ANIMATION.md). Materials convert to Lambert + harbor cel — never MeshStandard/MeshToon on iOS. |
