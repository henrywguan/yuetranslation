...
Scout characters: `scout-female.glb` / `scout-male.glb` (Meshy, unskinned A-pose — runtime auto-rig in `harborScoutRig.ts`). Frozen cast/reel on a new mesh → [`GLB-ANIMATION.md`](./GLB-ANIMATION.md).
