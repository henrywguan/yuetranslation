...
When keys are configured and you want to verify Azure/model paths (speech token, TTS with Free quota / Family unlimited metering, model MT, Solo UI):

密钥已配置、需要核验 Azure／模型通路（语音令牌、免费版配额／家庭版无限计量的朗读、模型翻译、独白界面）时：
