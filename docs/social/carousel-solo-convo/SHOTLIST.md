...
2. `node scripts/build-carousel-solo-convo.mjs` → Ken Burns + soft overlays + bed mux
3. Azure TTS (`POST /api/tts`) for slides 3 + 5 → remux with ducked bed
4. Export → `out/slide-0N-*.mp4` + `out/caption.txt`

**Theme note:** Dark / night mode only (Henry preference).

**TTS status:** Requires a real Azure Speech portal key (typically 32 hex chars). Vision key alone cannot authorize Speech TTS.
