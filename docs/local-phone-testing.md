...
- **Mic still blocked / no speech detected** — use the Cloudflare HTTPS tunnel (not `http://192.168.x.x`). Allow microphone when the browser asks. On iPhone, Solo/Conversation now unlock the mic on tap and prefer Web Speech. Set `AZURE_SPEECH_KEY` in `apps/api/.env` for Azure STT when possible. If you see “No speech detected yet…”, check permission and speak closer to the phone.
