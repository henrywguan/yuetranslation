...
export async function sendCampaignToRecipients(input: {
  ...
  fields: CampaignFields
  emails: string[]
}): Promise<{ sent: number; resendIds: string[] }> {
  const resend = getResend()
  ...
  await logSend({
    ...
    detail: { resendIds, errors: errors.slice(0, 20), attempted: emails.length },
  })

  if (!sent) throw new Error(errors[0] || 'Send failed')
  return { sent, resendIds }
}
