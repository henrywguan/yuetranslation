/**
 * Infer Lao register from source text — no user toggle.
 * Default: colloquial everyday Vientiane Lao (natural spoken).
 * Switch to formal when the source looks official, legal, medical, or ceremonial.
 */
export type LaoRegister = 'colloquial' | 'formal'
...
/** Detect register from the utterance being translated (usually English → Lao). */
export function inferLaoRegister(text: string): LaoRegister {
  const t = text.trim()
