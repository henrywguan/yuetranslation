/**
 * Infer Thai register from source text — no user toggle.
 * Default: colloquial everyday Central Thai (Bangkok-natural, spoken).
 * Switch to formal when the source looks official, legal, medical, or ceremonial.
 */
export type ThaiRegister = 'colloquial' | 'formal'
...
/** Detect register from the utterance being translated (usually English → Thai). */
export function inferThaiRegister(text: string): ThaiRegister {
  const t = text.trim()
