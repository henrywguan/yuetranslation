/**
 * Infer Peninsular Spanish register from source text — no user toggle.
 * Default: colloquial Castilian (Spain) street speech.
 * Switch to formal when the source looks official, legal, medical, or ceremonial.
 */
export type PeninsularSpanishRegister = 'colloquial' | 'formal'
...
export function inferPeninsularSpanishRegister(text: string): PeninsularSpanishRegister {
  const t = text.trim()
