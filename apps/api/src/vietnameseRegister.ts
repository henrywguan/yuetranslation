/**
 * Infer Vietnamese register from source text — no user toggle.
 * Default: colloquial everyday Vietnamese (Southern-friendly, natural spoken).
 * Switch to formal when the source looks official, legal, medical, or ceremonial.
 */
export type VietnameseRegister = 'colloquial' | 'formal'
...
/** Detect register from the utterance being translated (usually English → Vietnamese). */
export function inferVietnameseRegister(text: string): VietnameseRegister {
  const t = text.trim()
