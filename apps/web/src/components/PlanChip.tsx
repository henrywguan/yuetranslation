...
export function PlanChip() {
  ...
  useEffect(() => {
    ...
    if (prefs?.ttsVoiceEs) {
      const v = resolveEsVoice(prefs.ttsVoiceEs)
      setEsVoice(v)
      writeLocalEsVoice(v)
    }
    if (prefs?.ttsVoiceEses) {
      const v = resolveEsesVoice(prefs.ttsVoiceEses)
      setEsesVoice(v)
      writeLocalEsesVoice(v)
    }
    if (prefs?.ttsVoiceVi) {
      const v = resolveViVoice(prefs.ttsVoiceVi)
      setViVoice(v)
      writeLocalViVoice(v)
    }
    if (prefs?.username) {
