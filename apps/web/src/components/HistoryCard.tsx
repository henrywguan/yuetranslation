        ...
        <div className="history-card-meta">
          ...
          {isLatest ? (
            <span className="history-card-badge">
              <BiText copy={ui.historyLatest} size="sm" layout="inline" hideJp />
            </span>
          ) : null}
        </div>
        <button
          ...
          onClick={onToggle}
        >
          <BiText copy={expanded ? ui.historyCollapse : ui.historyExpand} size="sm" layout="inline" hideJp />
      ...
      {expanded ? (
        <div className="history-card-detail" id={`history-detail-${turn.id}`}>
          {turn.definition?.trim() ? (
            <p className="history-card-def">
              <span className="history-card-detail-label">
                <BiText copy={ui.definition} size="sm" layout="inline" hideJp />
              </span>
          ...
          {turn.alternatives?.length ? (
            <div className="history-card-alts">
              <p className="history-card-detail-label">
                <BiText copy={ui.historyVariations} size="sm" layout="inline" hideJp />
              </p>
              <ul>
          ...
          {hasDrill ? (
            ...
            >
              <BiText copy={ui.historyBreakdown} size="sm" layout="inline" hideJp />
            </button>
          ) : null}

          {!hasDetails && !hasDrill ? (
            <p className="history-card-empty-detail muted">
              <BiText copy={ui.historyNoDetails} size="sm" layout="inline" hideJp />
            </p>
          ) : null}
        </div>
      ) : null}
