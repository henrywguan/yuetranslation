...
function main() {
  ...
  assert.match(panelSrc, /speakManual/, 'pier hear chips auto-play TTS')
  ...
  assert.match(delveModalSrc, /speakManual/, 'delve auto-plays hearHan TTS')
  const autoSpeakSrc = readFileSync(new URL('../../lib/autoSpeakPref.ts', import.meta.url), 'utf8')
  assert.match(autoSpeakSrc, /v === null\) return true/, 'Auto-speak defaults ON when unset')
