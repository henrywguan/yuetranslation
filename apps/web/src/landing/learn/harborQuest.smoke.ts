...
function main() {
  ...
  assert.match(wornCss, /\.hq-visit-panel--inv[^{]*\{[^}]*bottom:\s*var\(--hq-explore-chrome\)/s, 'bag docks above explore chrome')
  ...
  assert.match(wornCss, /\.hq-visit-panel--inv[^{]*\{[^}]*max-height:\s*none/s, 'inv panel height from top+bottom pin')
