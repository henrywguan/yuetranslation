...
function main() {
  ...
  assert.equal(HARBOR_WEATHER_LOOK.sunny.ambI, 1.32, 'sunny ambient leaves room for key contrast')
  assert.equal(HARBOR_WEATHER_LOOK.sunny.sunI, 2.95, 'sunny sun key stronger than fill')
  assert.ok(HARBOR_WEATHER_LOOK.sunny.sunI > HARBOR_WEATHER_LOOK.sunny.ambI, 'sunny key > ambient for cel bands')
  assert.ok(HARBOR_WEATHER_LOOK.night.ambI < HARBOR_WEATHER_LOOK.sunny.ambI, 'night darker fill')
  assert.match(worldSrc, /attachHarborContactShadow|contact-shadow/, 'fake contact shadows under scouts')
  assert.match(worldSrc, /baseDistance/, 'lantern stores base distance for pool pulse')
  assert.match(worldSrc, /waterPhase \* 4\.2/, 'lantern intensity pulse')
