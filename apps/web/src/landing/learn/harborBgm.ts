...
function scheduleVoice(ctx: AudioContext, dest: AudioNode, v: Voice, t0: number) {
  ...
  if (v.kind === 'pluck') {
    master.gain.setValueAtTime(0.0001, t)
    master.gain.exponentialRampToValueAtTime(0.12 * v.gain, t + 0.01)
    master.gain.exponentialRampToValueAtTime(0.0001, t + dur)
    const o = ctx.createOscillator()
    o.type = 'triangle'
    o.frequency.setValueAtTime(v.freq, t)
  ...
  // flute — soft triangle + sine, light vibrato
  master.gain.setValueAtTime(0.0001, t)
  ...
  const filter = ctx.createBiquadFilter()
  filter.type = 'lowpass'
  filter.frequency.setValueAtTime(v.freq * 3.2, t)
  ...
  for (const [type, mul, g] of [
    ...
    lfoG.gain.setValueAtTime(v.freq * 0.006, t)
    lfo.connect(lfoG)
...
function scheduleLoop(ctx: AudioContext) {
  ...
  for (const v of HARBOR_BGM_PHRASE) {
...
/** Start looping Harbor BGM (idempotent). Call after a user gesture when possible. */
export function startHarborBgm(): void {
