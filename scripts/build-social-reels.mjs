#!/usr/bin/env node
/**
 * Build 22s Instagram Reels from B-roll (video or still) + UI screenshots + text overlays.
...
const clips = [
  {
    id: '01-dinner',
    brollVideo: join(OUT, 'broll-01-dinner.mp4'),
    brollStill: join(OUT, 'broll-01-dinner.png'),
  ...
  {
    id: '02-miss-you',
    brollVideo: join(OUT, 'broll-02-bedroom.mp4'),
    brollStill: join(OUT, 'broll-02-bedroom.png'),
  ...
  {
    id: '03-dont-worry',
    brollVideo: join(OUT, 'broll-03-kitchen.mp4'),
    brollStill: join(OUT, 'broll-03-kitchen.png'),
...
function buildDrawtext(lines) {
  return lines
    .map((l) => {
      ...
      return `drawtext=fontfile='${FONT}':text='${esc(l.text)}':fontsize=${l.size}:fontcolor=${col}:x=(w-text_w)/2:y=h*0.78:borderw=3:bordercolor=0x07131f@0.65:enable='${enable}'`
    })
    .join(',')
}

function resolveBroll(clip) {
  if (existsSync(clip.brollVideo)) return { path: clip.brollVideo, isVideo: true }
  if (existsSync(clip.brollStill)) return { path: clip.brollStill, isVideo: false }
  console.error('Missing broll for', clip.id)
  process.exit(1)
}
...
for (const clip of clips) {
  const broll = resolveBroll(clip)
  const tmpBroll = join(OUT, `tmp-${clip.id}-broll.mp4`)
  const tmpUi = join(OUT, `tmp-${clip.id}-ui.mp4`)
  const draw = buildDrawtext(clip.lines)

  if (broll.isVideo) {
    // Scale/crop cinematic B-roll to 14s (loop/trim as needed)
    execSync(
      `ffmpeg -y -stream_loop -1 -i '${broll.path}' -vf "scale=${W}:${H}:force_original_aspect_ratio=increase,crop=${W}:${H},fps=${FPS}" -t ${BROLL_SEC} -an -pix_fmt yuv420p '${tmpBroll}'`,
      { stdio: 'inherit' },
    )
  } else {
    execSync(
      `ffmpeg -y -loop 1 -i '${broll.path}' -vf "scale=${W}:${H}:force_original_aspect_ratio=increase,crop=${W}:${H},zoompan=z='min(zoom+0.0004,1.08)':x='iw/2-(iw/zoom/2)':y='ih/2-(ih/zoom/2)':d=${BROLL_SEC * FPS}:s=${W}x${H}:fps=${FPS}" -t ${BROLL_SEC} -pix_fmt yuv420p '${tmpBroll}'`,
      { stdio: 'inherit' },
    )
  }

  const uiIn = existsSync(clip.ui) ? clip.ui : broll.path
  execSync(
  ...
  console.log('Wrote', clip.out, broll.isVideo ? '(cinematic video broll)' : '(still broll)')
}
