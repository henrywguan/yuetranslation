-- Camera translation metering: seconds + translate cache-miss count.

alter table public.usage_months
  add column if not exists camera_seconds integer not null default 0;

alter table public.usage_months
  add column if not exists camera_translate_count integer not null default 0;
...
grant execute on function public.increment_usage(uuid, text, integer, integer, integer, integer, integer) to service_role;
