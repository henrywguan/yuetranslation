-- Atomic usage increments (avoids read-modify-write races that wipe sibling counters).
-- Also ensures translate_count exists on older databases.

alter table public.usage_months
  add column if not exists translate_count integer not null default 0;
...
begin
  ...
  -- usage_months FK → profiles; create a free profile if the auth user has none yet.
  insert into public.profiles (id, plan)
  values (p_user_id, 'free')
  on conflict (id) do nothing;

  insert into public.usage_months as u (user_id, month, live_seconds, tts_chars, translate_count)
  values (
    p_user_id,
    p_month,
...
grant execute on function public.increment_usage(uuid, text, integer, integer, integer) to service_role;
