-- AI vision LLM OCR fallback metering (view-only; no hard cap).

alter table public.usage_months
  add column if not exists ai_vision_count integer not null default 0;
...
grant execute on function public.increment_usage(uuid, text, integer, integer, integer, integer, integer, integer, integer) to service_role;
