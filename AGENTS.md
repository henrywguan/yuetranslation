...
Default deploy flags in `vercel.json`: `YUE_OPEN_MODE=0`, `YUE_REQUIRE_LOGIN=1`. Guests can use **text translate** and **tap-to-play voice** at `#/app` without signing in; live mic still requires login and is metered for signed-in users. Auto-speak remains Pro/Max.
