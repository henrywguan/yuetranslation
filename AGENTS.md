...
Default deploy flags in `vercel.json`: `YUE_OPEN_MODE=0`, `YUE_REQUIRE_LOGIN=1`. Guests can use **Solo text translate** and **tap-to-play voice** at `#/app` without signing in; Free TTS is metered with a hard char cap; Family/Max TTS is unlimited (usage still counted). Live mic still requires login and is metered. Auto-speak remains Family/Max. **Cam** (AR / Upload / Documents) requires login; camera minutes and document pages are **separate** meters — see [docs/entitlements.md](docs/entitlements.md) and [docs/camera.md](docs/camera.md). Production Family live minutes may be **20** via `YUE_FAMILY_LIVE_MINUTES` in `vercel.json` (not the code default of 60).
